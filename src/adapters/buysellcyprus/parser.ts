...
export function parseIndexPage(
  ...
  // Fallback: find links to property pages
  $('a[href]').each((_, el) => {
    const href = $(el).attr('href') || '';
    if (!href.match(/\/(real-estate|property|listing)\/.*\d/)) return;
...
export function parseDetailPage(
  ...
  const listingType: 'rent' | 'sale' = url.includes('/rent') || url.includes('/to-rent')
    ? 'rent' : 'sale';
